...
- `:::` 语法（Docusaurus/MDX）不被 GitHub 支持，使用引用块 `>` 替代
- 代码块必须指定语言，纯文本用 `text`
- 表格管道符需空格：`| ------ |` 而非 `|------|`
- 裸 URL 需用 `<>` 包裹或链接格式
...
- MD013（行长度）- 中文和 URL 不强制换行
- MD024（重复标题）- CHANGELOG 标准格式
- MD033（内联 HTML）- README 居中对齐
- MD041（首行 h1）- PR 模板无需顶级标题
...
- **静态异步原语**：使用 `LazyLock<Arc<T>>` 模式（如 `Semaphore`），便于在多个任务间克隆共享
- **spawn_blocking + timeout**：闭包内移动的变量需提前克隆用于外层错误处理；超时应包装 `JoinHandle` 而非同步调用
