...
- **图标工具栏**：
  ...
  - **主题**：点击月亮/太阳图标 <span class="material-icons-round">dark_mode</span> 可切换亮色/暗色模式
...
- **输出详细运行日志文件**：适用于 UPDFiler_v2，勾选后将输出详细的运行时日志文件，便于调试和深入分析
...
- **结果操作栏**：
  - **🧹 清除结果**：点击顶部扫帚图标可清空当前控制台的所有记录
  - **📋 复制日志**：点击顶部复制图标可将所有处理记录复制到剪贴板
  - **交互提示**：操作成功后会出现 Toast 提示（如"已清除所有结果"）
- **✅ 成功标识**：绿色对勾表示文件处理成功
