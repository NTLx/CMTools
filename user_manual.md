...
CMTools 是一款现代化、高性能的色谱数据处理软件。它无需安装即可运行（绿色软件），提供简洁直观的界面，助力高效批量处理数据文件。
...
*   **macOS版本 (Apple Silicon)**：下载 `CMTools.AppleSilicon.dmg`（暂不支持 Intel 芯片 Mac）。
...
3.  **处理选项区**：根据您选择的工具，自动显示可调节的参数（如"使用峰面积"、"标准品名称"等）。
...
_如果不确定该用哪个工具，请参考下文的_[_工具功能对照表_](#5-%E5%B7%A5%E5%85%B7%E5%8A%9F%E8%83%BD%E5%AF%B9%E7%85%A7%E8%A1%A8)_。_
...
*   **Tolerance (容差)**：仅 `STR-Matcher` 工具显示。用于设置匹配时的数值容差（需大于 0），留空则使用默认值。
...
| 工具名称 | 核心用途 | 是否支持峰面积 | 是否需标准品名称 |
| --- | --- | --- | --- |
| **AneuFiler** | 21、18、13和性染色体倍型检测试剂盒（荧光PCR-毛细管电泳法）产品配套分析工具 | ✅ | \- |
| **Aneu23** | 人类染色体非整倍体检测试剂盒(荧光PCR-毛细管电泳法)产品配套分析工具 | ✅ | ✅ |
| **SMNFiler\_v1** | 运动神经元存活基因拷贝数检测试剂盒（荧光PCR-毛细管电泳法）产品配套分析工具（旧版） | ✅ | ✅ |
| **SMNFiler\_v2** | 运动神经元存活基因拷贝数检测试剂盒（荧光PCR-毛细管电泳法）产品配套分析工具（新版） | ❌ | ✅ |
| **SHCarrier** | SMN1/2、HBA1/2和SLC25A13基因检测试剂盒（荧光PCR-毛细管电泳法）产品配套分析工具 | ✅ | ✅ |
| **UPDFiler\_v1** | 6，7，11，14和15号染色体STR分型试剂盒（荧光PCR-毛细管电泳法）产品配套分析工具（旧版） | ❌ | \- |
| **UPDFiler\_v2** | 6，7，11，14和15号染色体STR分型试剂盒（荧光PCR-毛细管电泳法）产品配套分析工具（新版） | ❌ | \- |
| **STR-Matcher** | STR 数据匹配与分析工具，支持自定义容差 (Tolerance) | ❌ | \- |
...
*   **32位系统**：`MicrosoftEdgeWebView2RuntimeInstallerX86.exe`
    
*   **64位系统**：`MicrosoftEdgeWebView2RuntimeInstallerX64.exe`
    
*   **ARM设备**：`MicrosoftEdgeWebView2RuntimeInstallerARM64.exe`
