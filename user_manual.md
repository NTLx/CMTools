...
_如果不确定该用哪个工具，请参考下文的 [工具功能对照表](#5-%E5%B7%A5%E5%85%B7%E5%8A%9F%E8%83%BD%E5%AF%B9%E7%85%A7%E8%A1%A8)_。_
...
A: CMTools 会收集**匿名的使用统计数据**，用于改进产品体验。我们非常重视您的隐私：
...
**隐私保护措施**：
- 采用 Cookieless 技术，不在您的电脑上存储追踪 Cookie
- 数据传输使用加密连接
- 您可以随时关闭遥测功能
...
A: 您可以通过以下方式控制遥测：
1. **首次启动**：软件会弹出授权提示，选择"拒绝"即可
2. **更改设置**：点击软件顶部栏右侧的**数据分析图标**（📊），按钮会在启用（深色）和禁用（浅色）状态间切换

关闭遥测后，软件的所有功能仍可正常使用，不会受到任何影响。
...
CMTools 尊重您的隐私权。我们仅在获得您明确授权后，才会收集匿名的使用数据，用于改进产品质量。

**收集的数据类型：**

| 事件类型 | 收集内容 | 用途 |
|---------|---------|-----|
| 工具使用统计 | 工具名称、处理文件数量 | 了解功能使用情况 |
| 性能指标 | 处理耗时、成功/失败数量 | 优化性能表现 |
| 错误报告 | 错误分类代码（不含个人数据） | 快速定位和修复问题 |
| 设置变更 | 语言、主题、遥测开关状态 | 改善用户体验 |
...
- 首次启动时会询问是否同意数据收集
- 可随时通过界面底部的"遥测"开关更改授权状态
- 拒绝数据收集不会影响软件的正常使用

> 我们采用 PostHog 的 cookieless 模式，通过服务端哈希计算用户 ID，进一步保护您的隐私。
